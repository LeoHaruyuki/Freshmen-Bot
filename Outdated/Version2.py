...
async def on_message(message):
    ...
    if message.author.bot == False:
        ...
        result = c.fetchone()
        if not result:
            c.execute("INSERT INTO totals (day, voice_seconds, messages) VALUES (?, ?, ?)", (cur_day, 0, 1))
            conn.commit()
...
async def on_voice_state_update(member, before, after):
    ...
    # TODO: Split time spent by days if it goes past midnight
        ...
        try:
            ...
            result = c.fetchone()
            if not result:
                c.execute("INSERT INTO count (id, messages, voice_seconds) VALUES (?, 0, 0)", (member.author.id,))
                conn.commit()
            c.execute("SELECT voice_seconds FROM count WHERE id = ?", (member.id,))
            result = c.fetchone()
